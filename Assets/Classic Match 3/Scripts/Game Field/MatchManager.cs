...
public class MatchManager : MonoBehaviour
{
    ...
    private List<MatchPieceType> pieceTypes = new List<MatchPieceType>();
	...
	public void Start()
	{
        ...
        Sprite[] sprites = Resources.LoadAll<Sprite>("Graphics/Shapes");
        foreach (Sprite s in sprites)
            pieceTypes.Add(new MatchPieceType() { name = s.name, sprite = s });

        matchPieceObject = Resources.Load<GameObject>("Prefabs/Tile");
	...
	{
		float startX = transform.position.x - xOffset * rows / 2 + xOffset / 2f;
		float startY = transform.position.y - yOffset * columns / 2 + yOffset / 2f;
	...
	public void SwapPieces(MatchPiece piece, SwapDirection direction, bool checkMatches = true)
	{
		...
		piece.ChangeSortingLayer("tileFront");
		otherPiece.ChangeSortingLayer("tileBack");
	...
	private bool CheckMatches(int x, int y, MatchesType type)
	{
		...
		List<MatchPiece> pieceList = new List<MatchPiece>(){ board[x][y] };
	...
	{
		pieceList.ForEach(x => {
			x.Explode(TIME_TO_EXPLODE);
		});

        Game.instance.AddPoints(pieceList.Count);
        Sound.instance.PlayClear();
	}
